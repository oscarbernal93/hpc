...
int main(int argc, char **argv)
{
	...
	mi_error = cudaMalloc((void**)&gpu_kernel,sizeof(char)*9);
	...
	mi_error = cudaMemcpy(gpu_kernel,cpu_kernel,sizeof(char)*9, cudaMemcpyHostToDevice);
	if(mi_error != cudaSuccess){printf("Error copiando kernel primero\n");exit(-1);}
	...
	nombre_resultado = strcat(nombre_imagen,".glob.jpg");
